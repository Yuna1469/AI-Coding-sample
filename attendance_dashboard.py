...
def generate_ai_insights(df, stats):
    ...
    try:
        # API 키 확인 (Streamlit secrets)
        api_key = st.secrets.get('GEMINI_API_KEY', None)
        
        if not api_key:
            return "⚠️ Gemini API 키가 설정되지 않았습니다. .streamlit/secrets.toml에 GEMINI_API_KEY를 설정해주세요."
        
        # 클라이언트 초기화
        client = genai.Client(api_key=api_key)
        
        # 데이터 요약 생성 (한글 인코딩 안전하게 처리)
        dept_stats_str = ""
        dept_summary = df.groupby('부서')['총 근무시간'].agg(['mean', 'count']).round(1)
        for dept, row in dept_summary.iterrows():
            dept_stats_str += f"- {dept}: 평균 {row['mean']}시간, {int(row['count'])}건\n"
        
        day_stats_str = ""
        day_summary = df.groupby('요일')['총 근무시간'].mean().round(1)
        for day, hours in day_summary.items():
            day_stats_str += f"- {day}: {hours}시간\n"
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        try:
            ...
            st.success(f"✅ '{selected_sheet}' 시트를 성공적으로 불러왔습니다!")
            st.write(f"📊 데이터 행 수: {len(df)}")
            st.write(f"📋 컬럼: {', '.join(df.columns.tolist())}")
            
        except Exception as e:
            st.error(f"❌ 파일을 읽는 중 오류가 발생했습니다: {str(e)}")
            st.info("💡 파일이 올바른 엑셀 형식(.xlsx, .xls)인지 확인해주세요.")
            df = None
    else:
        # 샘플 데이터 생성
...
if df is not None:
    ...
    with col3:
        ...
        dept_avg = df_processed.groupby('부서')['총 근무시간'].mean().round(1).to_dict()
        st.write(f"**부서별 평균 근무시간:** {dept_avg}")
    ...
    # API 키 설정 안내
    api_key_exists = st.secrets.get('GEMINI_API_KEY', None)
    if not api_key_exists:
        ...
        1. `.streamlit/secrets.toml` 파일을 생성 또는 열어주세요
        2. 다음 형식으로 API 키를 설정하세요:
        
        ```toml
        GEMINI_API_KEY = "your_api_key_here"
        ```
        
        3. 파일을 저장하고 Streamlit 앱을 재시작하세요
        
        **API 키 발급**: [Google AI Studio](https://aistudio.google.com/apikey)
        
        **참고**: `.streamlit/secrets.toml` 파일은 `.gitignore`에 자동으로 추가되어 안전합니다.
        """)
